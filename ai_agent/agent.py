...
async def chat_pipeline(query: str, user_id: str):
    ...
    try:
        ...
        logger.info(f"search output: {search_output[0]}")
        ...
        # Strategic questions
        strategic_question_template = f"Context information is below\nContext: {context}\n" + "Given the context information and not prior knowledge. Generate only Questions based on the context.\n\nYou are a Research Analyst/ strategist a professional who prepares investigative reports on the context provided for client use. Your task is to generate questions for an upcoming presentation on strategy building.\n\nRestrict your question generation based on the context information provided in a JSON format as follows:\n\n'{\"response\": \"True/False\", \"report_data\": \"[qiestion1, question2,..]\"} .\n"
        ...
        logger.info(f"Strategic question message: {strategic_question_message}")
        query_questions = await openai_manager.generate_text(strategic_question_message)
        logger.info(f"Query questions: {query_questions}")
        
        # Final report generation.
        template = f"Context information is below\nContext: {context}\n" + "Given the context information and not prior knowledge. Generate only answers based on the below query\n\nYou are a Research Analyst/ strategist a professional who prepares investigative reports on securities or assets for in-house or client use. Your task is to answer questions for an upcoming presentation on strategy building. MOreover, your task is to present a detailed report with insights in proper format.\n\nRestrict your answer based on the context information provided. If you dont know the answer then you can apply your prior knowledge and generate a detailed report. Share the response in JSON format as follows:\n\n {\"response\": \"True/False\", \"report_data\": \"html report/None\"}\n" + f"query: {query_questions}\n"

        messages = [
            {"role": "system", "content": template},
            {"role": "user", "content": f"Provide me a detailed strategic/competitor analysis report in HTML format.."}
        ]
